...
function ShelfSpines({ prefersReduced }: { prefersReduced: boolean | null }) {
  ...
  return (
    ...
    >
      ...
      >
        {SPINES.map((spine, i) => (
          /* Entry animation wrapper — handles rise-from-below on scroll */
          <motion.div
            key={spine.title}
            initial={{ y: prefersReduced ? 0 : 28, opacity: prefersReduced ? 1 : 0 }}
            whileInView={{
              y: spine.featured ? -8 : 0,
              opacity: 1,
            }}
            viewport={{ once: true, margin: "-8%" }}
            transition={{ duration: 0.55, delay: prefersReduced ? 0 : 0.15 + i * 0.09, ease: EASE }}
          >
            {/* Hover interaction wrapper — independent from entry */}
            <motion.div
              animate={{
                y: hoveredSpine === i && !prefersReduced
                  ? spine.featured ? -12 : -16
                  : 0,
                rotate: hoveredSpine === i && !prefersReduced
                  ? spine.lean * 0.4
                  : spine.lean,
              }}
              transition={{ type: "spring", stiffness: 420, damping: 28 }}
              onMouseEnter={() => setHoveredSpine(i)}
      ...
      {/* Spine info / shelf label — appears below the plank */}
      <div style={{ height: "2.5rem", marginTop: "0.6rem", display: "flex", alignItems: "center" }}>
        <AnimatePresence mode="wait">
          {hoveredSpine !== null && !prefersReduced ? (
            <motion.div
              key={hoveredSpine}
...
export function Books() {
  ...
  return (
    ...
    >
      ...
      {/* Book spines — section visual hero, top of section */}
      <div style={{ marginBottom: "clamp(4rem, 9vh, 7rem)" }}>
        <ShelfSpines prefersReduced={prefersReduced} />
      </div>

      {/* Staff pick card + reading list — two column on desktop, stacked on mobile */}
      <div
        ...
        >
          ...
          >
            ...
            >
              ...
              <div
                role="article"
                aria-label="Staff book recommendation"
                style={{
                  ...
                  transition: "transform 380ms cubic-bezier(0.25,0.1,0.25,1), box-shadow 380ms cubic-bezier(0.25,0.1,0.25,1)",
                  position: "relative",
                ...
                onMouseEnter={(e) => {
                  if (!prefersReduced) {
                    ...
                    e.currentTarget.style.boxShadow = "6px 14px 40px rgba(40,60,30,0.16), 0 2px 8px rgba(40,60,30,0.10), inset 0 1px 0 rgba(255,255,255,0.72)"
                  }
                }}
                onMouseLeave={(e) => {
                  if (!prefersReduced) {
                    e.currentTarget.style.transform = "rotate(-0.4deg)"
                    e.currentTarget.style.boxShadow = "3px 6px 24px rgba(40,60,30,0.10), 0 1px 4px rgba(40,60,30,0.06), inset 0 1px 0 rgba(255,255,255,0.72)"
                  }
                }}
              >
                <p style={{
                ...
                <p style={{
                  ...
                  The kind of book that makes the afternoon feel longer.
                  We&apos;ve had a copy near the counter for over a year.
                  It&apos;s been borrowed twelve times.
                </p>
                ...
                {!prefersReduced && (
                  <motion.div
                    ...
                    style={{
                      position: "absolute",
                      bottom: "clamp(1.125rem, 2.5vh, 1.5rem)",
